...
long double try_min_distance(vector<Point> &points, int left, int right);
...
long double try_min_distance(vector<Point> &points, int left, int right) {
  ...
  int mid = (left + right) / 2;
  long double d_left = try_min_distance(points, left, mid);
  long double d_right = try_min_distance(points, mid + 1, right);
  ...
  for (int i = 0; i < points_y_order.size(); i++) {
    for (int j = i + 1; j < points_y_order.size() && j <= i + 7; j++) {
      ...
      long double tmp_d = points_y_order[i].distance_to(points_y_order[j]);
...
int main() {
  ...
  std::cout << std::fixed;
  std::cout << std::setprecision(16) << minimal_distance(x, y) << "\n";
}
